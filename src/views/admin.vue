...
export default {
  ...
  data() {
    return {
      ...
      dropdowns: { ebook: false, products: false },
      activeLink: "",
      loading: true,
      error: "",
    };
  ...
  async mounted() {
    try {
      ...
      if (!token) {
        const loginRes = await axios.post(`${API_BASE}/admin/sign-in`, {
      ...
      this.stats = {
        ...
        ebooks: res.data.ebooks || 0,
        audiobooks: res.data.audiobooks || 0,
      };

      // if API provides categories, overwrite dummy ones
